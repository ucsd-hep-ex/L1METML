...
def main(args):
    ...
    predict_test = keras_model.predict(Xr_valid) * 50
    ...
    baseline_xy[:,0] = np.sum(Xr_valid[0][:,:,1]*Xr_valid[0][:,:,0], axis=1) * (-1)
    baseline_xy[:,1] = np.sum(Xr_valid[0][:,:,2]*Xr_valid[0][:,:,0], axis=1) * (-1)

    MakePlots(Yr_valid, predict_test, baseline_xy)
