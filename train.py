...
def test(Yr_test, predict_test, PUPPI_pt, path_out):
    ...
    Phi_abs_error_opaque(PUPPI_pt[:,1], predict_test[:,1], Yr_test[:,1])
    Pt_abs_error_opaque(PUPPI_pt[:,0], predict_test[:,0], Yr_test[:,0])
...
def trainFrom_Root(args):
    ...
    if quantized == True:
        ...
        keras_model = dense_embedding_quantized(n_features = n_features_pf, n_features_cat=n_features_pf_cat, n_dense_layers=3, activation_quantizer='quantized_tanh',embedding_input_dim = trainGenerator.emb_input_dim, number_of_pupcandis = 100, t_mode = t_mode, with_bias=False, logit_quantizer = 'quantized_bits', logit_total_bits=logit_total_bits, logit_int_bits=logit_int_bits, activation_total_bits=activation_total_bits, activation_int_bits=activation_int_bits, alpha=1, use_stochastic_rounding=False)
    else:
        keras_model = dense_embedding(n_features = n_features_pf, n_features_cat=n_features_pf_cat, n_dense_layers=3, activation='tanh',embedding_input_dim = trainGenerator.emb_input_dim, number_of_pupcandis = 100, t_mode = t_mode, with_bias=True)
...
def trainFrom_h5(args):
    ...
    if quantized == True:
        ...
        keras_model = dense_embedding(n_features = n_features_pf, n_features_cat=n_features_pf_cat, n_dense_layers=3, activation='tanh', embedding_input_dim = emb_input_dim, number_of_pupcandis = 100, t_mode = t_mode, with_bias=True)
