...
def preProcessing(A, normFac, EVT=None):
    ...
    dxyErr[np.where(np.abs(dxyErr) > 100)] = 0.0  # Cap extreme dxyErr values

    # Handle hcalDepth outliers and NaN/inf values
    hcalDepth[np.where(~np.isfinite(hcalDepth))] = 0.0  # Replace NaN/inf with 0
...
def MakePlots(trueXY, mlXY, puppiXY, path_out):
    ...
    nbins = 20
    binnings = np.linspace(
        0, 400, num=nbins + 1
    )  # create 20 bins for pt from 0 to 400 GeV
